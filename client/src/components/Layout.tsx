    ...
    <div className="flex h-screen bg-[#070d1a] overflow-hidden">
      <aside className="w-60 flex flex-col border-r border-[#1a3350] bg-[#0a1525] shrink-0">
        ...
        <nav className="flex-1 p-3 space-y-0.5">
          {nav.map(({ to, label, icon: Icon, exact, gold, purple }: any) => (
            <NavLink
              key={to}
              to={to}
              end={exact}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                  isActive && gold
                    ...
                    : purple
                    ? 'text-purple-500/70 hover:bg-purple-950/30 hover:text-purple-400 border border-transparent hover:border-purple-900/30'
